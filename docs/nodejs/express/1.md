...
中间件的本质就是一个回调函数，挂载方式主要分为 **全局挂载** 和 **局部挂载**

#### 1、直接挂载  <Badge>局部挂载</Badge>
...
// 回调数组形式
app.get('/example/c', [ middleWare1, middleWare2, callbackFun ]);
// 回调列表
app.get('/example/c', middleWare1, middleWare2, callbackFun);
// 混合
app.get('/example/c', [ middleWare1, middleWare2 ], callbackFun);
...
#### 2、app.use()  <Badge>局部挂载</Badge> <Badge>全局挂载</Badge>
