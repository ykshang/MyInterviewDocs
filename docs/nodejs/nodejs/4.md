...
- **安装（`install`）**：安装依赖包到项目下，可以安装指定名字、指定版本的依赖包，也可以根据`package.json`和`package-lock.json`文件安装项目所需的全部依赖包。其主要参数:
``` shell
package_name      依赖包名，可以通过package_name@1.2.10安装指定版本、也可以使用
                  package_name@4.x、package_name@4.1.x安装某个范围内的最新版本。

--save, -s        安装到项目下，添加到dependencies

--save-dev, -d    安装到项目下，添加到 devDependencies
